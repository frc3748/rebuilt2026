...
public class VisionSubsystem extends StateMachine<VisionSubsystem.State> {
    ...
    private final CameraInputsAutoLogged turretCamera =
        new CameraInputsAutoLogged();
    private final CameraInputsAutoLogged chassisCamera =
        new CameraInputsAutoLogged();
    ...
    public void update() {
        ...
        if (getState() == State.BROKEN) return;
        ...
        // Weighted Fusion
        Optional<VisionFieldPoseEstimate> fused = fuseEstimates(
            turretEst,
            chassisEst
        );

        if (fused.isPresent()) {
            if (getState() != State.VISION_SCANNING) setState(
                State.VISION_SCANNING
            );
    ...
    private Optional<VisionFieldPoseEstimate> processCamera(
        ...
        if (!cam.seesTarget) return Optional.empty();

        // 1. Pick the best estimate (Prefer Megatag 2)
        boolean useMT2 =
            cam.megatag2PoseEstimate != null && cam.megatag2Count > 0;
        var estimate = useMT2
            ? cam.megatag2PoseEstimate
        ...
        // 2. Timing Check
        if (
            timestamp == (isTurret ? lastTurretTimestamp : lastChassisTimestamp)
        ) return Optional.empty();

        // 3. Motion Validation (Method fully implemented below)
        if (
            !isMotionValid(timestamp, isTurret, logPath)
        ) return Optional.empty();
        ...
        if (isTurret) lastTurretTimestamp = timestamp;
        else lastChassisTimestamp = timestamp;
     ...
     */
    private Optional<Transform2d> getRobotToCamera(
        double timestamp,
        boolean isTurret
    ) {
        if (isTurret) {
            // Retrieve the historical rotation of the turret from the RobotState buffer
            Optional<Rotation2d> turretRotation = state.getRobotToTurret(
                timestamp
            );

            if (turretRotation.isPresent()) {
                // Combine: Robot -> Turret (Historical) -> Camera (Static offset on turret)
                Transform2d robotToTurret = MathHelpers.transform2dFromRotation(
                    turretRotation.get()
                );
                return Optional.of(
                    robotToTurret.plus(state.getTurretToCamera(true))
                );
            }
            return Optional.empty();
        }
    ...
    private boolean isMotionValid(
        ...
        var chassisVel = state.getMaxAbsDriveYawAngularVelocityInRange(
            timestamp - kWindow,
            timestamp
        );
        ...
        if (isTurret) {
            var turretVel = state.getMaxAbsTurretYawAngularVelocityInRange(
                timestamp - kWindow,
                timestamp
            );
            totalAngularVel = Math.abs(
                chassisVel.orElse(0.0) + turretVel.orElse(0.0)
            );
        }
    ...
    private Optional<VisionFieldPoseEstimate> fuseEstimates(
        Optional<VisionFieldPoseEstimate> turret,
        Optional<VisionFieldPoseEstimate> chassis
    ) {
        if (turret.isEmpty()) return chassis;
        if (chassis.isEmpty()) return turret;
        ...
        var varianceA = a
            ...
            .elementTimes(a.getVisionMeasurementStdDevs());
        var varianceB = b
            .getVisionMeasurementStdDevs()
            .elementTimes(b.getVisionMeasurementStdDevs());

        Rotation2d fusedHeading = poseB.getRotation();
        if (
            varianceA.get(2, 0) < VisionConstants.kLargeVariance &&
        ...
        Matrix<N3, N1> fusedStdDev = VecBuilder.fill(
            ...
            Math.sqrt(
                1.0 / (1.0 / varianceA.get(2, 0) + 1.0 / varianceB.get(2, 0))
            )
        );
        ...
        return Optional.of(
            new VisionFieldPoseEstimate(fusedPose, time, fusedStdDev, numTags)
        );
    }
